...
public class ReactAgent extends BaseAgent {
    ...
    /**
     * 将智能体转换为异步节点操作（支持多字段输出）
     *
     * @param inputKeyFromParent 从父节点获取输入的键
     * @param outputKeys 输出到父节点的多个字段名（子图和父图使用相同字段名）
     * @return 异步节点操作实例
     * @throws GraphStateException 图状态异常
     */
    public AsyncNodeAction asAsyncNodeAction(String inputKeyFromParent, List<String> outputKeys)
            throws GraphStateException {
        return asAsyncNodeAction(inputKeyFromParent, outputKeys, null);
    }
    
    /**
     * 将智能体转换为异步节点操作（支持多字段输出 + 保留中间结果）
     *
     * @param inputKeyFromParent 从父节点获取输入的键
     * @param outputKeys 输出到父节点的多个字段名（子图和父图使用相同字段名）
     * @param preserveKeys 需要保留的中间结果字段（这些字段会在迭代过程中累积保存）
     * @return 异步节点操作实例
     * @throws GraphStateException 图状态异常
     */
    public AsyncNodeAction asAsyncNodeAction(String inputKeyFromParent, List<String> outputKeys, List<String> preserveKeys)
            throws GraphStateException {
        if (this.compiledGraph == null) {
            this.compiledGraph = getAndCompileGraph();
        }
        return node_async(new SubGraphStreamingNodeAdapter(inputKeyFromParent, outputKeys, this.compiledGraph, preserveKeys));
    }
...
//					state.updateState(Map.of(this.inputKey, messages));
    ...
    public static class SubGraphStreamingNodeAdapter implements NodeAction {
        ...
        private final List<String> outputKeys;  // 多个输出字段名
        
        private final List<String> preserveKeys;  // 需要保留的中间结果字段

        private final CompiledGraph childGraph;

        /**
         * 构造函数（单字段输出）
         * @param inputKeyFromParent 从父图读取输入的字段名
         * @param outputKeyToParent 输出到父图的主字段名（会输出文本）
         * @param childGraph 子图
         */
         ...
         * 构造函数（多字段输出）
         * @param inputKeyFromParent 从父图读取输入的字段名
         * @param outputKeys 输出到父图的多个字段名
         * @param childGraph 子图
         */
        public SubGraphStreamingNodeAdapter(String inputKeyFromParent, List<String> outputKeys,
                                            CompiledGraph childGraph) {
            this(inputKeyFromParent, outputKeys, childGraph, null);
        }
         ...
         */
        public SubGraphStreamingNodeAdapter(String inputKeyFromParent, List<String> outputKeys,
                                            CompiledGraph childGraph, List<String> preserveKeys) {
            this.inputKeyFromParent = inputKeyFromParent;
            this.outputKeyToParent = null;
        ...
        @Override
        public Map<String, Object> apply(OverAllState parentState) throws Exception {
            // 从父状态中获取传入的 inputKeyFromParent 数据
            ...
            AsyncGenerator<NodeOutput> wrapped = new AsyncGenerator<>() {
                ...
                private final Map<String, Object> preservedFields = new HashMap<>();  // 累积保留的字段
                ...
                public Data<NodeOutput> next() {
                    ...
                    if (data.isDone()) {
                        // 子图结束，合并保留的字段和最终状态
                        Map<String, Object> outputData = buildOutputData(lastStateData, preservedFields);
                        return Data.done(outputData);
                    }
                    if (data.isError()) {
                        return data;
                    }
                    return Data.of(data.getData().thenApply(n -> {
                        try {
                            ...
                            // 累积保留指定的字段
                            if (preserveKeys != null && lastStateData != null) {
                                for (String key : preserveKeys) {
                                    if (lastStateData.containsKey(key)) {
                                        preservedFields.put(key, lastStateData.get(key));
                                    }
                                }
                            }
            ...
            // 返回包装的 Generator
            // 多字段模式：使用固定的临时 key，最终会被 buildOutputData 的结果替换
            // 单字段模式：使用 outputKeyToParent
            String wrapperKey = (outputKeyToParent != null) ? outputKeyToParent : "__multi_output__";
            return Map.of(wrapperKey, wrapped);
        }

        /**
         * 构建输出数据，根据配置决定返回单个字段还是多个字段
         * 
         * @param stateData 最终状态数据
         * @param preservedFields 累积保留的字段
         */
        private Map<String, Object> buildOutputData(Map<String, Object> stateData, Map<String, Object> preservedFields) {
            if (outputKeys != null) {
                // 多字段模式：合并保留的字段和最终状态
                return extractMultipleFields(stateData, preservedFields);
            } else {
                // 单字段模式（向后兼容）
                String text = extractAssistantText(stateData);
                return Map.of(outputKeyToParent, text);
            }
        }
         ...
         * 
         * 特殊处理：
         * - 如果字段是 "messages"，提取最后一条助手消息的文本
         * - 其他字段优先从 preservedFields 中获取，其次从 stateData 中获取
         * 
         * @param stateData 最终状态数据
         * @param preservedFields 累积保留的字段
         */
        private Map<String, Object> extractMultipleFields(Map<String, Object> stateData, Map<String, Object> preservedFields) {
            Map<String, Object> result = new HashMap<>();

            if (stateData == null && (preservedFields == null || preservedFields.isEmpty())) {
                return result;
            }
            ...
            outputKeys.forEach(key -> {
                if ("messages".equals(key)) {
                    // 特殊处理 messages 字段：提取最后一条助手消息的文本
                    String text = extractAssistantText(stateData);
                    if (!text.isEmpty()) {
                        result.put(key, text);
                    }
                } else {
                    // 其他字段：优先从 preservedFields 获取，其次从 stateData 获取
                    Object value = null;
                    if (preservedFields != null && preservedFields.containsKey(key)) {
                        value = preservedFields.get(key);
                    } else if (stateData != null && stateData.containsKey(key)) {
