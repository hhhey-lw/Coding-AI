...
//                )
    ...
    public ReactAgent buildAgent() throws GraphStateException {
        ...
        LlmNode llmNode = LlmNode.builder()
                .systemPrompt("""
                        ...
                        行动: <你的下一步行动>。
                        观察：<总结工具调用的结果>，可选，仅使用了工具才需要回复该部分。
                        ...
                        用户：请你生成一张海滩落日照片。
                        助手：
                        	思考：用户想要一张海滩落日的照片，但没有提供参考图。因为generateImage函数需要一个参考图片的URL地址作为参数，我将采取默认的处理方式，即不依赖于特定的参考图来生成新的图片。
                        		 接下来，首先，由于用户提供的提示词过于简单，我需要先丰富和优化用户提示词。然后，我会使用generateImage工具并仅提供描述性的提示词来生成图片。
                        	行动：generateImage(prompt="海滩落日，金色阳光洒在海面上，天空中有绚丽的晚霞，宁静而美丽的氛围", referenceImage="")。
                        	<注意：如果需要使用工具，你必须明确给出工具调用的消息请求，这个要求你必须严格遵守，否则不会使用工具>
                        工具响应：图片URL。
                        助手：
                        	观察：我已经成功得到了工具生成的海滩落日照片，图片URL如下：http://www.test.com/example.png。
                        	思考：我已经生成了用户需要的海滩落日照片。
                        	行动：本次任务完成。
                        ### 注意
                        - 每次只能调用一个工具。
                        - 如果你需要使用工具完成对应的任务，除了回复思考、行动外，你需要在消息中回复工具调用的请求。
                        - 你必须遵守格式，即思考和行动是一对，观察只有使用了工具时你才需要简要总结工具回复。
                        - 避免重复回复结果，避免冗长的解释和前端重复渲染。
                        - 如果调用了工具，得到了工具的结果，你需要补充观察标签，然后继续思考和行动。
                        - 你需要严格按照上述格式进行回复。
                        """)
    ...
    public CompiledGraph buildPlanExecuteAgent() throws GraphStateException {
        this.planningClient = ChatClient.builder(this.chatModel)
