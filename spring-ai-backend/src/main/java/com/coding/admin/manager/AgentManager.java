...
//                )
    ...
    public CompiledGraph buildPlanExecuteAgent() throws GraphStateException {
        ...
        ReactAgent chatAgent = ReactAgent.build(chatModel, ReactAgent.builder()
                .name("qa_agent")
                .modelName("qwen-plus")
        ...
        StateGraph graph = new StateGraph("plan_execute_graph", stateFactory)
                .addNode("qa_agent", chatAgent.asAsyncNodeAction("messages", "messages"))
                .addNode("planning_agent", planningAgent.asAsyncNodeAction("messages", "plan"))
                ...
                .addConditionalEdges(START, new RoutingEdgeAction(chatModel, "你是一个意图分析大师，能够根据用户的提问，选择最合适的Agent，帮助用户完成任务", "messages",
                        List.of(chatAgent, planningAgent)), Map.of("qa_agent", "qa_agent", "planning_agent", "planning_agent"))
                .addEdge("qa_agent", END)
