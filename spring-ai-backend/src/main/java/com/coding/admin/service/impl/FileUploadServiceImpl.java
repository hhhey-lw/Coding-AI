...
import com.coding.admin.enums.ImageFormatEnum;
...
import com.coding.admin.utils.Md5Utils;
...
public class FileUploadServiceImpl implements FileUploadService {
     ...
     */
    @Override
    public FileUploadResponseModel uploadImage(MultipartFile file) {
        // 验证文件
        ...
        // 检查文件是否已存在
        FileInfoDO existingFile = fileInfoRepository.getByMd5(fileMd5);
        ...
        try {
            ...
            // 保存文件信息到数据库
            FileInfoDO fileInfoDO = buildFileInfo(file, fileName, fileUrl, fileMd5);
            Long fileId = fileInfoRepository.save(fileInfoDO);
            if (fileId == null) {
     ...
     * @return 文件上传响应
     */
    private FileUploadResponseModel buildUploadResponse(String fileName, String fileUrl, MultipartFile file) {
     ...
     * @return FileInfo对象
     */
    private FileInfoDO buildFileInfo(MultipartFile file, String fileName, String fileUrl, String fileMd5) {
        FileInfoDO fileInfoDO = new FileInfoDO();
        fileInfoDO.setFileName(fileName);
        fileInfoDO.setOriginalName(file.getOriginalFilename());
        ...
        fileInfoDO.setCreateTime(LocalDateTime.now());
        return fileInfoDO;
    }
     ...
     * @return 文件上传响应
     */
    private FileUploadResponseModel buildResponseFromFileInfo(FileInfoDO fileInfoDO) {
        FileUploadResponseModel response = new FileUploadResponseModel();
        response.setFileName(fileInfoDO.getFileName());
        response.setFileUrl(fileInfoDO.getFileUrl());
        response.setFileSize(fileInfoDO.getFileSize());
        response.setContentType(fileInfoDO.getContentType());
        response.setUploadTime(System.currentTimeMillis());
        return response;
    }
