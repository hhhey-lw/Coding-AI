...
import com.coding.workflow.model.chat.ModelCredential;
...
public class ModelFactory {
    ...
    public ChatModel getChatModel(String provider, String serviceType) {
        // 从数据库获取提供商配置
        ProviderConfigDO providerConfig = providerConfigService.getByProviderCodeAndServiceType(provider, serviceType);
     ...
     * 构建OpenAI API客户端
     */
    private OpenAiApi buildOpenAiApi(ProviderConfigDO providerConfigDO) {
        OpenAiApi.Builder openAiApiBuilder = OpenAiApi.builder()
                .apiKey(providerConfigDO.getAuthorization())
        ...
        if (StrUtil.isNotBlank(providerConfigDO.getBaseUrl())) {
            String endpoint = providerConfigDO.getBaseUrl();
