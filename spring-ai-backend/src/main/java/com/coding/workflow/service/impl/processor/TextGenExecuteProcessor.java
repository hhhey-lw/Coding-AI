...
public class TextGenExecuteProcessor extends AbstractExecuteProcessor {
    ...
    public NodeResult innerExecute(DirectedAcyclicGraph<String, Edge> graph, Node node, WorkflowContext context) {
        ...
        Mono<AgentResponse> chatResponse = modelExecuteManager.chat(modelConfig.getProvider(), "TextGen", modelConfig.getModelId(), paramMap, requestMessages);
        ...
        if (agentResponse.isSuccess()) {
            ...
            if (StrUtil.isBlank(responseText)) {
                ...
                nodeResult
                        .setError(ErrorCodeEnum.WORKFLOW_EXECUTE_ERROR.toError("模型调用没有返回任何内容"));
            }
            else {
